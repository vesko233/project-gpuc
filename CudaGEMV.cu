...
// Naive kernel
__global__
void kernelST(float *vec, float *mat, float *out, const unsigned int N, const unsigned int M)
{
...
{
    float *dev_input, *dev_matrix, *dev_output;
    ...
    for(int i = 0; i < M; ++i) output[i] = biases[i];
    ...
    kernelRD<<<dimGrid, dimBlock, THREADS_PER_BLOCK*sizeof(float)>>>(dev_input, dev_matrix, dev_output, N, M);

    // Naive kernel
    // kernelST<<<1,M>>>(dev_input, dev_matrix, dev_output, N, M);
